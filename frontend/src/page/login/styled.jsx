/**
 * Login - componentes específicos (vidro, bolas)
...
export function FormArea({ children, className = '', ...props }) {
  return (
    <form
      className={`flex flex-col items-center gap-5 w-full ${className}`.trim()}
      {...props}
    >
      {children}
    </form>
...
export function LoginInputRow({ icon, action, children, className = '', ...props }) {
  return (
    <div
      className={`flex items-center gap-3 w-full h-14 rounded-full border border-white/50 bg-white/30 backdrop-blur-sm overflow-hidden px-4 ${className}`.trim()}
      {...props}
    >
